	...
	public function build(array $classes, array $field_map): array|\WP_Error {
		...
		foreach ($classes as $class) {
			...
			foreach ((array) ($class['cres'] ?? array()) as $cre) {
				...
				$seen[$cre] = true;
				try { $representations[] = new Resource_Representation($cre, (string) $field_map[$cre]['id'], $key, $identity); } catch (\InvalidArgumentException) { return $this->error('turmas_bridge_invalid_resource_plan', 'O plano de Resource da Turma é inválido.'); }
			}
		...
		return array_values(array_map(static fn (Resource_Plan $plan): array => $plan->to_array(), $plans));
	}
