...
final class Publication_Controller {
	...
	public function __construct(?Publication_Command_Store $store = null) { $this->store = $store ?? new Idempotency_Repository(); }
	...
	public function receive(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
		...
		$response = array('schema_version' => Publication_Payload::SCHEMA_VERSION, 'publication_key' => (string) $payload['publication']['publication_key'], 'status' => 'accepted', 'idempotent_replay' => false);
		if (! $this->store->record($key, $hash, 202, $response)) {
