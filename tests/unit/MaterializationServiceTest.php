...
final class MaterializationServiceTest extends TestCase {
	...
	public function test_ambiguous_existing_reservation_without_form_id_never_creates_a_second_clone(): void {
		...
		$store->records['2027:MT1'] = array('publication_key' => '2027:MT1', 'template_id' => 199, 'form_id' => null, 'status' => 'RECEIVED');
		$gateway = new Fake_Gravity_Gateway();

		$result = (new Materialization_Service($store, $gateway))->materialize($this->payload(), 'a');
	...
	public function test_failure_before_reservation_can_retry_after_the_template_becomes_valid(): void {
		...
		$service = new Materialization_Service($store, $gateway);
	...
	public function test_clone_failure_leaves_an_ambiguous_reservation_blocked_for_reconciliation(): void {
		...
		self::assertSame('turmas_bridge_reconciliation_required', $service->materialize($this->payload(), 'b')->get_error_code());
		self::assertSame(0, $gateway->duplicates);
	}
...
final class Fake_Gravity_Gateway implements Gravity_Forms_Gateway {
	public bool $available = true; public bool $materialized_form_missing = false; public bool $clone_fails = false; /** @var array<string,mixed>|null */ public ?array $template = array('id' => 199, 'is_active' => true, 'fields' => array(array('id' => 10, 'choices' => array(array('text' => 'x'))), array('id' => 11, 'choices' => array(array('text' => 'y'))))); public int $duplicates = 0;
